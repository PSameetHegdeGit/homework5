...
class Planner(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        #feed logit into spatial_argmax to get the aimpoint
        return spatial_argmax(torch.squeeze(logit, dim=1))
...
if __name__ == '__main__':
    from homework.controller import control
    ...
    def test_planner_local():
        # Load model
        planner = load_model().eval()
        pytux = PyTux()
        for t in ['zengarden']:
            steps, how_far = pytux.rollout(t, control, planner=planner, max_frames=1000, verbose=True)
            print(steps, how_far)
        pytux.close()
