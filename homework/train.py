...
def train(args, data_folder='drive_data'):
    ...
    train_data = load_data(f'{data_folder}/train', num_workers=4, batch_size=32) #TODO: Apply transform later (likely just color jitter and maybe horizontal flip?)
    valid_data = load_data(f'{data_folder}/valid', num_workers=4, batch_size=32)
    ...
    for epoch in range(args.num_epoch):
        ...
        for img, true_aim_point in train_data:
            ...
            if train_logger is not None:
                ...
                train_logger.add_scalar('accuracy', accuracy(detected_aim_point.cpu().detach().numpy(), true_aim_point.cpu().detach().numpy()))
            optimizer.zero_grad()
            l.backward()
            optimizer.step()
            global_step += 1
        ...
        with torch.no_grad():
            for img, true_aim_point in valid_data:
                ...
                if valid_logger is not None:
                    ...
                    train_logger.add_scalar('accuracy', accuracy(detected_aim_point.cpu().detach().numpy(), true_aim_point.cpu().detach().numpy()), global_step)
                global_step += 1


        print('epoch %-3d \t training loss %0.3f \t validation loss %0.3f' % (epoch, sum(train_loss)/len(train_loss), sum(valid_loss)/len(valid_loss)))
        save_model(model)
